...
public class FormBarang extends JFrame {
    ...
    FormBarang() {
        ...
        txkode_barang.setBounds(105, 10, 175, 20);
    ...
    void simpanData(tbl_barang barang) {
        try {
            Connection cn = new connectDB().getConnect();
            Statement st = cn.createStatement();
            String sql = "INSERT INTO tbl_barang (kode_barang, nama_barang, harga_barang, stok_barang) " +
                    "VALUES ('" + barang.getkode_barang() + "', '" + barang.getnama_barang() + "', '" + barang.getharga_barang() + "','" + barang.getstok_barang() + "')";
            int result = st.executeUpdate(sql);
            cn.close();
            JOptionPane.showMessageDialog(null, "Data Berhasil Disimpan",
                    "Info Proses", JOptionPane.INFORMATION_MESSAGE);
            String[] data = {barang.getkode_barang(), barang.getnama_barang(), barang.getharga_barang(), barang.getstok_barang()};
            df.addRow(data);
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
    ...
    void ubahData(tbl_barang barang, String kode_barang) {
        try {
            Connection cn = new connectDB().getConnect();
            Statement st = cn.createStatement();
            String sql = "UPDATE tbl_barang SET kode_barang='" + barang.getkode_barang() + "', nama_barang='" + barang.getnama_barang() + "', harga_barang='" + barang.getharga_barang() + "', stok_barang='" + barang.getstok_barang() + "' WHERE kode_barang ='" + kode_barang + "'";
            int result = st.executeUpdate(sql);
            cn.close();
    ...
    private void btaddAksi(ActionEvent evt) {
        ...
        barang.setkode_barang(txkode_barang.getText());
        barang.setnama_barang(txnama_barang.getText());
        barang.setharga_barang(txharga_barang.getText());
        barang.setstok_barang(txstok_barang.getText());
        simpanData(barang);
    }
